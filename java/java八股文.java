...
hashmap 初始加载容量（16）为什么一定是2的n次方 如果初始容量是10 底层会无符号右移也会变成2的n次方
因为在计算插入数据的插入位置时候hash&（length-1） 2的n次方实际就是最后一位是个0，2的n次方-1实际最后一位就是1；
相同的二进制数位上，都是1的时候，结果为1，否则为零 （如果最后一位是0 那么最后一位&出来的肯定是0）。 
